...
# Loading data
csec_data <-read.csv("Plot_Data_Ahmed,Li,Marr-Laing.csv")
...
plot <- ggplot(csec_long, aes(x = start_year, y = value, color = variable, group = variable)) +
  geom_line(aes(x = start_year, y = value, color = variable, group = variable), linewidth = 1.1) +
  geom_point(data = point_data, aes(x = start_year, y = value, color = variable, group = variable), size = 2.5) +
  theme_bw() +
  ...
  theme(axis.text = element_text(size = 18),
        ...
        axis.title.x = element_text(margin = margin(t=20), size = 20, face = "bold", hjust = 0.598),
  ...
  geom_segment(aes(x = 2017, y = -0.055, yend = 0.01), color = "black", linewidth = 1.25) +
  geom_vline(xintercept = 2020, linetype = "dashed", color = "black", size = 0.7) +
  geom_text(aes(x = 2017, y = -0.05, label = text_label), hjust = -0.05, color = "black", size = 5) + 
  theme(text = element_text(family = "Calibri")) +
  scale_color_manual(labels = expression(bold("C-section rate") ~ "per 100 births", 
